...
def quadratic_equel (par1, par2, par3):
    ...
    dict_result['a'] = check_param(par1)
    dict_result['b'] = check_param(par2)
    dict_result['c'] = check_param(par3)
    if dict_result['a'] == 0:
    ...
    if type(dict_result['a']) is int and type(dict_result['b']) is int and type(dict_result['c']) is int:
        ...
        if dict_result['d'] <0:
            ...
            dict_result['x_1'] = get_eq_root(dict_result['a'], dict_result['b'], dict_result['d'])
            dict_result['x_2'] = get_eq_root(dict_result['a'], dict_result['b'], dict_result['d'], 0)

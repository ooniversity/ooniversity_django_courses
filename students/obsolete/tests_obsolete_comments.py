...
class StudentTests(TestCase):
    ...
    def test_students_list(self): 
        ...
        #self.assertContains(response, 'StudentName1')#already checked
        #self.assertContains(response, 'StudentName2')#already checked
        #self.assertContains(response, 'STUDENTSURNAME1')#upper! already checked
        #self.assertContains(response, 'STUDENTSURNAME2')#upper! already checked
        #self.assertContains(response, 'address1')#already checked
        #self.assertContains(response, 'address2')#already checked
        #self.assertContains(response, 'scype1')#already checked
        #self.assertContains(response, 'scype2')#already checked
        ...
        self.assertEqual(student1.email, 'email1@email.email')#no need
        self.assertEqual(student2.email, 'email2@email.email')#no need
...
#need it really??? M2M!!! not evident?!
    ...
    def test_student_detail(self):
        ...
        """
        interim_date_format = ", ".join(student1.birth_date.split("-"))
        format_birth_date = datetime.datetime.strptime(
            interim_date_format,
            '%Y, %m %d').date()      
        self.assertContains(response, format_birth_date)
        #ValueError: time data '1988, 01, 11' does not match format '%Y, %m %d'
        """
